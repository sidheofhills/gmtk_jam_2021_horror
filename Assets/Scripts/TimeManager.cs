...
public class TimeManager : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if(KnockUI.HeKnocked)
        {
            ...
            {
                EventManager.TriggerEvent("gameWinEvent");  // 'they gonna get here in an hour!'  and make notifications like 30 min left, 15 min left, 10 min left
            }
        }
        ...
        if (clockTimer <= 0)
        {
            ...
            Minute++;  //+1
            EventManager.TriggerEvent("onMinuteEvent");
            if (Minute >=60)
            {
                Hour++;    //+1
                Minute = 0;
                EventManager.TriggerEvent("onHourEvent");
            }
            else if (Minute%2==0 && KnockUI.HeKnocked)
            {
                EventManager.TriggerEvent("onTwoMinuteEvent");  // MAYBE TEN IS WAY TOo SLOW AND A MINUTE WILL DO
            }
